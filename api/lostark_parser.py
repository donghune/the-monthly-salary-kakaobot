...
def parse_basic(data: dict):
    return f"""
서버 : {data["Basic"]["Server"]} | 클래스 : {data["Basic"]["Class"]["Name"]} | 길드 : {data["Basic"]["Guild"]} | 칭호 : {data["Basic"]["Title"]} |
"""


def parse_level(data: dict):
    return f"""
캐릭터 : {data["Basic"]["Level"]["Battle"]} | 아이템 : {data["Basic"]["Level"]["Item"]} | 원정대 : {data["Basic"]["Level"]["Expedition"]} | 영지 : {data["Basic"]["Wisdom"]["Level"]} | 
        """
...
def parse_stat(data: dict):
    return f"""
공격력 : {data["Basic"]["Stat"]["Attack"]} | 최대 생명력 : {data["Basic"]["Stat"]["Health"]} | 치명 : {data["Basic"]["Stat"]["Critical"]} | 특화 : {data["Basic"]["Stat"]["Specialty"]} | 신속 : {data["Basic"]["Stat"]["Agility"]} | 제압 : {data["Basic"]["Stat"]["Proficiency"]} | 인내 : {data["Basic"]["Stat"]["Endurance"]} | 
        """
...
def parse_get_collections(data: dict):
    return f"""
섬의 마음 : {data["Collections"][0]["섬의 마음"]} {parse_collection_complete(data["Collections"][0]["섬의 마음"])} |  오르페우스의 별 : {data["Collections"][1]["오르페우스의 별"]} {parse_collection_complete(data["Collections"][1]["오르페우스의 별"])} |  거인의 심장 : {data["Collections"][2]["거인의 심장"]} {parse_collection_complete(data["Collections"][2]["거인의 심장"])} |  위대한 미술품 : {data["Collections"][3]["위대한 미술품"]} {parse_collection_complete(data["Collections"][3]["위대한 미술품"])} |  모코코 씨앗 : {data["Collections"][4]["모코코 씨앗"]} {parse_collection_complete(data["Collections"][4]["모코코 씨앗"])} |  항해 모험물 : {data["Collections"][5]["항해 모험물"]} {parse_collection_complete(data["Collections"][5]["항해 모험물"])} |  이그네아의 징표 : {data["Collections"][6]["이그네아의 징표"]} {parse_collection_complete(data["Collections"][6]["이그네아의 징표"])} |  세계수의 잎 : {data["Collections"][7]["세계수의 잎"]} {parse_collection_complete(data["Collections"][7]["세계수의 잎"])} |  
"""
